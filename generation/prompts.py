...
def _r11(p: dict) -> str:
    ...
    n = p.get("min_common", 2)
    return f"""아래는 한 리더에 대한 동료·구성원·상사의 주관식 응답입니다.
작성자를 특정할 단서(시점·소속·자기지칭·어투)를 한 차례 걸렀지만, **문장 자체가
아직 작성자를 드러냅니다.** 말버릇, 겪은 사건, 관계가 그대로 남아 있습니다.

원문 응답 ({p.get('label', '주관식')}):
{items}

할 일: **{n}명 이상이 공통으로 말한 주제만** 골라, 리더 본인이 읽을 문장으로
다시 쓰십시오. 요약이 아니라 **재작성**입니다.

**재작성 방식 — 이렇게 바꿉니다**

| 응답 원문 | 리포트에 실을 문장 |
|---|---|
| "배려가 많으신 만큼 R&R이 애매할 때가 있어요" | 팀원들의 사정에 대해 배려가 많은 리더십이나, 업무 R&R 분장에 좀 더 신경 써 줄 필요가 있음 |
| "1on1이 자주 밀려요ㅠ" | 업무가 바쁘더라도 팀원들과의 일정을 우선순위에 두고 고려할 필요가 있음 |

바뀐 것을 보십시오 — 감정 어미와 말버릇이 사라지고, 겪은 사건이 일반화되고,
불평이 **관찰 + 제언**으로 바뀌었습니다. 뜻은 그대로입니다.

**규칙**
1. {n}명 이상이 공통으로 말한 주제가 있으면 그것을 씁니다. 없으면 **억지로
   묶지 말고 한 줄씩 나열합니다.** 없는 공통점을 지어내는 것이 더 나쁩니다.
   단, 나열할 때도 위 재작성 방식을 그대로 지키고 몇 명이 말했는지는 쓰지
   않습니다 — 응답 수가 적을 때는 그 숫자가 사람을 좁힙니다.
2. 특정 사건·회의·프로젝트·날짜·횟수를 쓰지 마십시오. "지난 워크숍에서", "세 번 연속"
   같은 대목은 그것만으로 작성자가 특정됩니다. 일반화하거나 지웁니다.
3. **응답의 표현을 그대로 옮기지 마십시오.** 인용부호 안의 말, 특유의 말투, 이모티콘,
   구어체를 남기면 발화자가 드러납니다.
4. 관찰과 제언으로만 씁니다. 비난·평가·추측을 쓰지 않습니다.
   `~할 필요가 있음`, `~면 좋을 것으로 보임` 같은 담백한 제언체로 끝냅니다.
5. 주제 하나당 한 줄. 최대 3줄. 줄바꿈으로 구분합니다.
6. 응답에 없는 사실을 만들지 않습니다.

evidence 에는 판단 근거가 된 원문 구절을 **주제마다 하나씩** 넣으십시오
(그대로 옮긴 것이어야 합니다 — 검수용이며 리포트에는 실리지 않습니다)."""
