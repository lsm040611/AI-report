...
class TypeConfirm(BaseModel):
    type: str | None = None       # 담당자가 판정을 바꿀 수도 있다
    operator: str
...
def get_card(card_id: int, db: Session = Depends(get_db)):
    ...
    return {"card": card.card_json, "sendable": ok, "blocked_by": reason}
...
def resolve_flag(card_id: int, index: int, body: FlagResolve,
    ...
    return {"ok": True, "sendable": ok, "blocked_by": reason}
...
def resolve_person(card_id: int, person_id: str, operator: str,
                   db: Session = Depends(get_db)):
    ...
    data["person"] = person
    card.card_json, card.person_id = data, person_id
    ...
    if not db.query(PersonResolution).filter(
        ...
        db.add(PersonResolution(memo_key=memo_key, person_id=person_id))
    db.commit()
    return {"ok": True}
